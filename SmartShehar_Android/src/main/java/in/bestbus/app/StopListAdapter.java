...
public class StopListAdapter extends CursorAdapter {
    CGlobals_BA mApp;
    int curId;
    ...
    public StopListAdapter(Context context, Cursor c, CGlobals_BA app) {
        super(context, c);
        mApp = app;
...
//              tv.setText(cursor.getString(COLUMN_DISPLAY_NAME));
        tv.setText(cursor.getString(cursor.getColumnIndexOrThrow("searchstr")).trim());
    }

    @Override
    public String convertToString(Cursor cursor) {
        curId = cursor.getInt(cursor.getColumnIndexOrThrow("_id"));
        return cursor.getString(cursor.getColumnIndexOrThrow("searchstr")).trim();
    }

    @Override
    public Cursor runQueryOnBackgroundThread(CharSequence constraint) {
        if (constraint != null)
            return mApp.mDBHelperBus.getStopNamesCursor(constraint.toString());
        else
            return null;
